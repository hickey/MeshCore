...
#else
  // Linux: remove every file directly under the base dir (flat layout, no subdirs).
  extern const char* file_shim_get_base_dir();
  DIR* d = opendir(file_shim_get_base_dir());
  if (!d) return false;
  struct dirent* ent;
  char path[512];
  while ((ent = readdir(d)) != NULL) {
    if (ent->d_name[0] == '.') continue; // skip "." and ".."
    snprintf(path, sizeof(path), "%s/%s", file_shim_get_base_dir(), ent->d_name);
    remove(path);
  }
  closedir(d);
  return true;
